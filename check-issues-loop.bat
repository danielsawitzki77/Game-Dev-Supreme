...
:loop
REM Determine current hour (PST) for interval selection
REM %time% gives local time; assuming system is set to PST
for /f "tokens=1 delims=: " %%h in ("%time%") do set /a HOUR=%%h

REM Quiet hours: 22:00 (10pm) through 07:59 (8am)
if %HOUR% GEQ 22 (
    set INTERVAL_SEC=%INTERVAL_QUIET%
) else if %HOUR% LSS 8 (
    set INTERVAL_SEC=%INTERVAL_QUIET%
) else (
