...
call :cleanup_branches "c:\Users\Daniel Sawitzki\Desktop\github\Artsy_2000"
...
kiro-cli chat --no-interactive --trust-all-tools "FIRST: Read the file c:\Users\Daniel Sawitzki\Desktop\github\Game-Dev-Supreme\cli-steering-combined.md - it contains all project steering docs (issue workflow, visual testing pipeline, SDL/picojson library context, game design docs). Follow ALL instructions in that file as your operational guide. THEN: Check all monitored GitHub repos for open issues. Run: gh issue list --repo danielsawitzki77/Zeitgeist-Evolved --state open --json number,title,labels,assignees,createdAt, then gh issue list --repo danielsawitzki77/SDL_VisualTest --state open --json number,title,labels,assignees,createdAt, then gh issue list --repo danielsawitzki77/Super-Civ-16 --state open --json number,title,labels,assignees,createdAt, then gh issue list --repo danielsawitzki77/TerrorForm --state open --json number,title,labels,assignees,createdAt, then gh issue list --repo danielsawitzki77/Game-Dev-Supreme --state open --json number,title,labels,assignees,createdAt, then gh issue list --repo danielsawitzki77/Artsy_2000 --state open --json number,title,labels,assignees,createdAt. %PICKUP_INSTRUCTION% ALSO: Scan ALL open PRs across all repos for unprocessed human comments (see Scanning ALL Open PRs section in steering). Run gh pr list --state open for each repo and check comments on any open PRs. If no actionable issues AND no open PR comments are found, just report that and stop. SIGNAL: If you found no actionable work, write the word NO_WORK to the file: %MARKER_FILE% — IMPORTANT: Write NO_WORK ONLY if you did NOT process any issues or PR comments at all during this session. If you processed at least one item (posted comments, made commits, pushed code), do NOT write the NO_WORK file even if the queue is now empty afterward."
